...
class LoadingButtonState extends State<LoadingButton>
  ...
  void _sendSMS() async {
    String message = "${UserData.username} is in DANGER, Help!";
    String smsUri;
    String? emergencyNumber = UserData.emer;

    // Properly encode the message to avoid invalid characters
    String encodedMessage = Uri.encodeComponent(message);

    // Check if an emergency number is provided
    if (emergencyNumber.isNotEmpty) {
      smsUri = "sms:$emergencyNumber?body=$encodedMessage";
    } else {
      // Some SMS apps may not handle empty numbers, so we'll prompt the user
      smsUri = "sms:?body=$encodedMessage";
    }

    try {
      // Use canLaunchUrl instead of canLaunchUrlString to ensure compatibility
      if (await canLaunchUrl(Uri.parse(smsUri))) {
        await launchUrl(Uri.parse(smsUri));
      } else {
        _showError("No available app to send SMS");
      }
    } catch (e) {
